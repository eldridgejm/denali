...
namespace denali {
    ...
    class ReadableDirectedGraphMixin : public Super
    {
        ...
        bool isNodeValid(Node node) const { return _graph.isNodeValid(node); }

        /// \brief Checks if the arc is valid.
        bool isArcValid(Arc arc) const { return _graph.isArcValid(arc); }

        /// \brief Gets the first node from the graph.
        Node getFirstNode() const { return _graph.getFirstNode(); }

        /// \brief Gets the next node from the graph.
        /// \pre Requires that the node is valid.
        Node getNextNode(Node node) const { return _graph.getNextNode(node); }

        /// \brief Gets the first arc from the graph.
        Arc getFirstArc() const { return _graph.getFirstArc(); }

        /// \brief Gets the next arc from the graph.
        /// \pre Requires that the arc is valid.
        Arc getNextArc(Arc arc) const { return _graph.getNextArc(arc); }

        /// \brief Gets the first arc into the node.
        /// \pre The node must be valid.
        Arc getFirstInArc(Node node) const { return _graph.getFirstInArc(node); }

        /// \brief Gets the next arc into the node.
        /// \pre The arc must be valid.
        Arc getNextInArc(Arc arc) const { return _graph.getNextInArc(arc); }

        /// \brief Get the source of the arc.
        /// \pre The arc must be valid.
        Node source(Arc arc) const { return _graph.source(arc); }

        /// \brief Gets the first arc out of the node.
        /// \pre The node must be valid.
        Arc getFirstOutArc(Node node) const { return _graph.getFirstOutArc(node); }

        /// \brief Gets the next arc out of the node.
        /// \pre The arc must be valid.
        Arc getNextOutArc(Arc arc) const { return _graph.getNextOutArc(arc); }

        /// \brief Get the target of the arc.
        /// \pre The arc must be valid.
        Node target(Arc arc) const { return _graph.target(arc); }

        /// \brief Gets the first neighbor arc attached to the node.
        /// \pre The node must be valid.
        Arc getFirstNeighborArc(Node node) const { return _graph.getFirstNeighborArc(node); }

        /// \brief Gets the next neighbor arc attached to the node.
        /// \pre The node must be valid, and the arc must be attached to it.
        Arc getNextNeighborArc(Node node, Arc arc) const { return _graph.getNextNeighborArc(node, arc); }

        /// \brief Get the opposite node of the arc.
        /// \pre The node must be in the arc.
        Node opposite(Node node, Arc arc) const { return _graph.opposite(node, arc); }

        /// \brief Get the degree of the node.
        /// \pre The node must be valid.
        unsigned int degree(Node node) const { return _graph.degree(node); }

        /// \brief Get the in degree of the node.
        /// \pre The node must be valid.
        unsigned int inDegree(Node node) const { return _graph.inDegree(node); }

        /// \brief Get the out degree of the node.
        /// \pre The node must be valid.
        unsigned int outDegree(Node node) const { return _graph.outDegree(node); }

        /// \brief The number of nodes in the graph
        unsigned int numberOfNodes() const { return _graph.numberOfNodes(); }

        /// \brief The number of arcs in the graph
        unsigned int numberOfArcs() const { return _graph.numberOfArcs(); }

        /// \brief Gets the maximum node identifier in the graph.
        unsigned int getMaxNodeIdentifier() const { return _graph.getMaxNodeIdentifier(); }

        /// \brief Get the identifier of the node.
        unsigned int getNodeIdentifier(Node node) const { return _graph.getNodeIdentifier(node); }

        /// \brief Gets the maximum arc identifier in the graph.
        unsigned int getMaxArcIdentifier() const { return _graph.getMaxArcIdentifier(); }

        /// \brief Get the identifier of the arc.
        unsigned int getArcIdentifier(Arc arc) const { return _graph.getArcIdentifier(arc); }

        /// \brief Finds an arc in the graph with the given source and target.
        /*!
         *  If no such arc exists, an invalid arc is returned.
         */
        Arc findArc(Node source, Node target) const { _graph.findArc(source, target); }
    ...
    class ReadableUndirectedGraphMixin : public Super
    {
        ...
        bool isEdgeValid(Edge edge) const { return _graph.isEdgeValid(edge); }
        ...
        Edge getFirstEdge() const { return _graph.getFirstEdge(); } 

        /// Gets the next edge from the graph.
        Edge getNextEdge(Edge edge) const { return _graph.getNextEdge(edge); }
        ...
        Node u(Edge edge) const { return _graph.u(edge); }

        /// \brief Gets the "v" node at the end of an edge.
        /// \pre The edge must be valid.
        Node v(Edge edge) const { return _graph.v(edge); }

        /// \brief The number of nodes in the graph
        unsigned int numberOfNodes() const { return _graph.numberOfNodes(); }

        /// \brief The number of arcs in the graph
        unsigned int numberOfEdges() const { return _graph.numberOfEdges(); }

        /// \brief Finds an edge in the graph with the given source and target.
        /*!
         *  If no such edge exists, an invalid edge is returned.
         */
        Edge findEdge(Node u, Node v) const { return _graph.findEdge(u,v); }
    ...
    public:
        ...
        Arc addArc(Node u, Node v) { return _graph.addArc(u,v); }
        ...
        ReadableUndirectedGraphMixin <WritableReadableUndirectedGraph, Super>
    {
        ...
        Edge addEdge(Node u, Node v) { return _graph.addEdge(u,v); }

        /// \brief Remove a node from the graph.
        /// \pre The node must be in the graph.
        void removeNode(Node node) { return _graph.removeNode(node); }

        /// \brief Remove an edge from the graph.
        /// \pre The edge must be in the graph.
        void removeEdge(Edge edge) { return _graph.removeEdge(edge); }
