...
namespace denali {
    ...
    class VectorDirectedGraphImplementation
    {
        ...
        class Arc
        {
            ...
            public:
            Arc() {}
            bool operator==(const Arc& arc) const { return index == arc.index; }
            bool operator!=(const Arc& arc) const { return index != arc.index; }
        };
        ...
        int numberOfNodes() const { return number_of_nodes; }
        int numberOfArcs() const { return number_of_arcs; }
    ...
    class UndirectedGraphImplementation
    {
        ...
        class Node
        {
            ...
            Node(typename Impl::Node _base) { base = _base; }

            public:
            Node() {}
            bool operator==(const Node& node) { return base == node.base; }
            bool operator!=(const Node& node) { return base != node.base; }
        };
        ...
        class Edge
        {
            ...
            Edge(typename Impl::Arc _base) { base = _base; }

            public:
            Edge() {}
            bool operator==(const Edge& edge) { return base == edge.base; }
            bool operator!=(const Edge& edge) { return base != edge.base; }
        };
        ...
        bool isNodeValid(Node node) const { return impl.isNodeValid(node.base); }
        bool isEdgeValid(Edge edge) const { return impl.isArcValid(edge.base); }
        Node getFirstNode() const { return Node(impl.getFirstNode()); }
        Node getNextNode(Node node) const { return Node(impl.getNextNode(node.base)); }
        Edge getFirstEdge() const { return Edge(impl.getFirstArc()); }
        Edge getNextEdge(Edge edge) const { return Edge(impl.getNextArc(edge.base)); }
        ...
        unsigned int degree(Node node) const { return impl.degree(node.base); }
        Node u(Edge edge) const { return impl.source(edge.base); }
        Node v(Edge edge) const { return impl.target(edge.base); }

        unsigned int numberOfNodes() const { return impl.numberOfNodes(); }
        unsigned int numberOfEdges() const { return impl.numberOfArcs(); }
        ...
        Node addNode() { return Node(impl.addNode()); }
        Edge addEdge(Node u, Node v) 
            { return Edge(impl.addArc(u.base, v.base)); }

        void removeNode(Node node) { impl.removeNode(node.base); }
        void removeEdge(Edge edge) { impl.removeArc(edge.base); }

        void clearNodes() { impl.clearNodes(); }
        void clearEdges() { impl.clearArcs(); }

        void attachNodeObserver(Observer& ob) { impl.attachNodeObserver(ob); }
        void detachNodeObserver(Observer& ob) { impl.detachNodeObserver(ob); }
        ...
        Edge findEdge(Node u, Node v)
        {
            ...
            if (impl.outDegree(u.base) < impl.outDegree(v.base)) {
                smaller = u.base; bigger = v.base;
            } else {
                smaller = v.base; bigger = u.base;
            }
